...
bool set_member(set *t, unsigned int key) {
    for (node *n = t->buckets[hash(key)]->next; n != NULL; n = n->next) {
        if (n->key == key) {
            return true;
        }
    }
...
void set_delete(set *t, unsigned int key) {
    for (node *n = t->buckets[hash(key)]; n->next != NULL; n = n->next) {
        if (n->next->key == key) {
            node *del = n->next;
            n->next = del->next;
            free(del);
...
void set_delete_all(set *t) {
    for (int b = 0; b < NUM_BUCKET; ++b) {
        while (t->buckets[b]->next != NULL) {
            node* p = t->buckets[b]->next;
            t->buckets[b]->next = p->next;
            free(p);
        }
    }
    t->size = 0;
